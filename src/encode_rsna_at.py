...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        #train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[["AT"]].values, dtype=torch.float32), transform)
        train_dataset = datasets.MILPathDataset(train_df.path.values, list(zip(torch.tensor(labels_df[["AT"]].values, dtype=torch.float32), labels_df["instance_label"].values)), transform)
    ...
    #train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[["AT"]].values, dtype=torch.float32), transform)
    train_dataset = datasets.MILPathDataset(train_df.path.values, list(zip(torch.tensor(train_df[["AT"]].values, dtype=torch.float32), train_df["instance_label"].values)), transform)
    #val_dataset = datasets.MILPathDataset(val_df.path.values, torch.tensor(val_df[["AT"]].values, dtype=torch.float32), transform)
    val_dataset = datasets.MILPathDataset(val_df.path.values, list(zip(torch.tensor(val_df[["AT"]].values, dtype=torch.float32), val_df["instance_label"].values)), transform)
    #test_dataset = datasets.MILPathDataset(test_df.path.values, torch.tensor(test_df[["AT"]].values, dtype=torch.float32), transform)
    test_dataset = datasets.MILPathDataset(test_df.path.values, list(zip(torch.tensor(test_df[["AT"]].values, dtype=torch.float32), test_df["instance_label"].values)), transform)
    ...
    for image, length, label in train_dataset:
        ...
        if label == 0.0:
            while True:
                sampled_length_labels = train_df[train_df["AT"] == 1.0].sample(n=1).iloc[0].instance_label
                indices = np.linspace(0, len(sampled_length_labels) - 1, len(embeddings)).round().astype(int)
    ...
    for image, length, label in val_dataset:
        ...
        if label == 0.0:
            while True:
                sampled_length_labels = val_df[val_df["AT"] == 1.0].sample(n=1).iloc[0].instance_label
                indices = np.linspace(0, len(sampled_length_labels) - 1, len(embeddings)).round().astype(int)
                length_labels = [sampled_length_labels[i] for i in indices]
                if sum(length_labels) != 0:
                    break            
    ...
    for image, length, label in test_dataset:
        ...
        if label == 0.0:
            while True:
                sampled_length_labels = test_df[test_df["AT"] == 1.0].sample(n=1).iloc[0].instance_label
                indices = np.linspace(0, len(sampled_length_labels) - 1, len(embeddings)).round().astype(int)
                length_labels = [sampled_length_labels[i] for i in indices]
                if sum(length_labels) != 0:
                    break            
