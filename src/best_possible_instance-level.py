...
if __name__=="__main__":
    ...
    parser.add_argument("--kernel_size", default=12, help="Kernel size (default: 12)", type=int)
    ...
    train_data["lengths_y"] = [torch.tensor(length_labels, dtype=torch.float32).view(-1, 1) for length_labels in train_data["lengths_y"]]
    val_data["lengths_y"] = [torch.tensor(length_labels, dtype=torch.float32).view(-1, 1) for length_labels in val_data["lengths_y"]]
    test_data["lengths_y"] = [torch.tensor(length_labels, dtype=torch.float32).view(-1, 1) for length_labels in test_data["lengths_y"]]
    
    train_dataset = datasets.MILTensorDataset(train_data["X"], train_data["lengths"], train_data["lengths_y"])
    val_dataset = datasets.MILTensorDataset(val_data["X"], val_data["lengths"], val_data["lengths_y"])
    test_dataset = datasets.MILTensorDataset(test_data["X"], test_data["lengths"], test_data["lengths_y"])
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=utils.instance_level_collate_fn)
    ...
    model = models.InstanceLevelClassifier(in_features=train_data["X"].shape[1], out_features=1, kernel_size=args.kernel_size)
