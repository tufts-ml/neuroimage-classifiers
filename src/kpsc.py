...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description="kpsc.py")
    ...
    parser.add_argument("--epochs", default=1_000, help="Number of epochs (default: 1_000)", type=int)
    parser.add_argument("--experiments_dir", default="", help="Directory to save experiments (default: \"\")", type=str)
    parser.add_argument("--label", default="CBI", help="Label (default: \"CBI\")", type=str)
    ...
    assert args.label in ["CBI", "WMD"]
    label_idx = ["CBI", "WMD"].index(args.label)
    train_dataset = datasets.MILTensorDataset(train_data["X"], train_data["lengths"], train_data["y"][:,label_idx].unsqueeze(1))
    val_dataset = datasets.MILTensorDataset(val_data["X"], val_data["lengths"], val_data["y"][:,label_idx].unsqueeze(1))
    test_dataset = datasets.MILTensorDataset(test_data["X"], test_data["lengths"], test_data["y"][:,label_idx].unsqueeze(1))
    ...
    if args.embedding_level:
        model = models.PoolClf(in_features=train_data["X"].shape[1], out_features=1, pooling=args.pooling)
    else:
        model = models.ClfPool(in_features=train_data["X"].shape[1], out_features=1, pooling=args.pooling)
