...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[['idCBI']].values, dtype=torch.float32), transform)
    ...
    train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[['idCBI']].values, dtype=torch.float32), transform)
    val_dataset = datasets.MILPathDataset(val_df.path.values, torch.tensor(val_df[['idCBI']].values, dtype=torch.float32), transform)
    test_dataset = datasets.MILPathDataset(test_df.path.values, torch.tensor(test_df[['idCBI']].values, dtype=torch.float32), transform)
