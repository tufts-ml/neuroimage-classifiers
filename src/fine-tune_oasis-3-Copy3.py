...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[['idWMD']].values, dtype=torch.float32), transform)
    ...
    train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[['idWMD']].values, dtype=torch.float32), transform)
    val_dataset = datasets.MILPathDataset(val_df.path.values, torch.tensor(val_df[['idWMD']].values, dtype=torch.float32), transform)
    test_dataset = datasets.MILPathDataset(test_df.path.values, torch.tensor(test_df[['idWMD']].values, dtype=torch.float32), transform)
    ...
    if args.model == "ViT-B/16":
        ...
        encoder.conv_proj.weight.data = encoder.conv_proj.weight.data.sum(dim=1, keepdim=True).repeat(1, args.num_channels, 1, 1)
        encoder.conv_proj.in_channels = args.num_channels
        encoder.heads = torch.nn.Identity()
        model = models.PoolClf(in_features=768, out_features=1, encoder=encoder, pooling=args.pooling, neighbors=args.neighbors)
