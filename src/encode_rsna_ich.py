...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        #train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[["ICH"]].values, dtype=torch.float32), transform)
        train_dataset = datasets.MILPathDataset(train_df.path.values, list(zip(torch.tensor(labels_df[["ICH"]].values, dtype=torch.float32), labels_df["Any"].values)), transform)
    ...
    #train_dataset = datasets.MILPathDataset(train_df.path.values, torch.tensor(train_df[["ICH"]].values, dtype=torch.float32), transform)
    train_dataset = datasets.MILPathDataset(train_df.path.values, list(zip(torch.tensor(train_df[["ICH"]].values, dtype=torch.float32), train_df["Any"].values)), transform)
    #val_dataset = datasets.MILPathDataset(val_df.path.values, torch.tensor(val_df[["ICH"]].values, dtype=torch.float32), transform)
    val_dataset = datasets.MILPathDataset(val_df.path.values, list(zip(torch.tensor(val_df[["ICH"]].values, dtype=torch.float32), val_df["Any"].values)), transform)
    #test_dataset = datasets.MILPathDataset(test_df.path.values, torch.tensor(test_df[["ICH"]].values, dtype=torch.float32), transform)
    test_dataset = datasets.MILPathDataset(test_df.path.values, list(zip(torch.tensor(test_df[["ICH"]].values, dtype=torch.float32), test_df["Any"].values)), transform)
...
#         if label == 0.0:
#             while True:
#                 sampled_length_labels = train_df[train_df["ICH"] == 1.0].sample(n=1).iloc[0].Any
...
#         if label == 0.0:
#             while True:
#                 sampled_length_labels = val_df[val_df["ICH"] == 1.0].sample(n=1).iloc[0].Any
#                 indices = np.linspace(start=0, stop=len(sampled_length_labels) - 1, num=len(embeddings)).round().astype(int)
#                 length_labels = [sampled_length_labels[i] for i in indices]
#                 if sum(length_labels) != 0:
#                     break            
...
#         if label == 0.0:
#             while True:
#                 sampled_length_labels = test_df[test_df["ICH"] == 1.0].sample(n=1).iloc[0].Any
#                 indices = np.linspace(start=0, stop=len(sampled_length_labels) - 1, num=len(embeddings)).round().astype(int)
#                 length_labels = [sampled_length_labels[i] for i in indices]
#                 if sum(length_labels) != 0:
#                     break            
